...
public class KafkaUtils {
    ...
    public static CommunicationAdapter createKafkaCommunicationClass(String executionId, String bootstrapServers) throws Exception {
        Map<String, Object> config = new HashMap<>();
        config.put("KAFKA_BOOTSTRAP_SERVERS_CONFIG", bootstrapServers);
        config.put("KAFKA_POLL_TIME_OUT", 2L);
        config.put("KAFKA_MAX_INITIALIZATION_TIME", 7L);
        config.put("KAFKA_MAX_ITERATION_TIME", 7L);
        config.put("KAFKA_MAX_MODEL_SIZE", 1);
        return new KafkaCommunication(config, executionId, 2);
    }
